...
namespace DB_Scheme_Extract.Utility
{
    public class DBSQL
    {
        ...
        /*****************************/
        public static string QUERY_TABLE_COLS = "SELECT COLUMN_NAME,DATA_TYPE,DATA_LENGTH,DATA_PRECISION,DATA_SCALE,NULLABLE,DEFAULT_LENGTH,DATA_DEFAULT,LAST_ANALYZED,CHAR_USED,COLUMN_ID FROM ALL_TAB_COLUMNS WHERE OWNER = 'MS9DJA' AND UPPER(TABLE_NAME) = UPPER('?') ";
        public static string QUERY_CONST_COLS = "SELECT B.CONSTRAINT_NAME, B.COLUMN_NAME FROM ALL_CONSTRAINTS A, ALL_CONS_COLUMNS B WHERE A.CONSTRAINT_NAME = B.CONSTRAINT_NAME AND A.OWNER = 'MS9DJA' AND UPPER(A.TABLE_NAME) = UPPER('?') AND A.CONSTRAINT_TYPE = 'P'";
        ...
        /*******************************************/
        /* below for generate preload table script
        /*******************************************/
        public static string QUERY_TABLE_COLUMNS = "SELECT COLUMN_NAME,DATA_TYPE FROM ALL_TAB_COLUMNS WHERE OWNER = 'MS9DJA' AND UPPER(TABLE_NAME) = UPPER('?') ORDER BY COLUMN_ID ASC ";
        public static string QUERY_COLUMNS_VALUES = "SELECT * FROM ? ";
    }
}